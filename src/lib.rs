...
use orbiter_rs::{
    ...
    OrbiterVessel, PropellantHandle, ReferenceFrame, SDKVessel, ThrusterGroupType, ThrusterHandle,
    Vector3, VesselStatus, V,
};
...
impl Surveyor {
    ...
    fn apply_thrusters(&self, f_1: f64, f_2: f64, f_3: f64, theta_1: f64) {
        ...
        self.ctx.SetThrusterDir(self.th_vernier[0], thruster1_dir);
    }
    ...
    fn compute_thrust_from_ang_acc(
        &self,
        vehicle_mass: f64,
        reference_thrust_level: f64,
        angular_acc: &Vector3,
        min_thrust_level: f64,
    ) -> (f64, f64, f64, f64) {
        if angular_acc.length() < 0.001 {
            return (
                reference_thrust_level,
                reference_thrust_level,
                reference_thrust_level,
                0.,
            );
        ...
        (
            F_1,
            reference_thrust_level + F_2,
            reference_thrust_level + F_3,
            theta_1,
        )
    }
    ...
    fn attitude_controller(&self) -> Vector3 {
        match &self.attitude_mode {
            ...
            AttitudeMode::InertialLock(target_orientation_global) => {
                ...
                self.ctx
                    .Global2Local(&target_orientation_global, &mut target_orientation);
                let (rotation_axis, rotation_angle) = self.compute_rotation(&target_orientation);
                self.pointing_controller(rotation_axis, rotation_angle)
            }
        }
    ...
    fn compute_target_vector_for_gravity_turn(&self) -> Vector3 {
        ...
        self.ctx
            .GetAirspeedVector(ReferenceFrame::Local, &mut airspeed);
    ...
    fn get_surface_approach_vel(&self) -> f64 {
        ...
        self.ctx
            .GetAirspeedVector(ReferenceFrame::Horizon, &mut rel_vel);
    ...
    fn on_pre_step(&mut self, _sim_t: f64, sim_dt: f64, _mjd: f64) {
        ...
        if self.descent_phase == DescentPhase::BeforeRetroIgnition {
            ...
            if altitude < RETRO_IGNITION_ALTITUDE {
                ...
                debug_string!(
                    "Waiting to fire retro ... altitude = {:.2} mi",
                    altitude / MI_IN_M
                );
            }
            ...
            if altitude < ENGINE_CUTOFF_ALTITUDE {
                ...
                self.ctx
                    .Local2Global(&DIR_Z_PLUS, &mut target_orientation_global);
        ...
        let reference_thrust = if self.descent_phase == DescentPhase::AfterRetro
            && altitude > ENGINE_CUTOFF_ALTITUDE
        {
            let delta_thrust = if altitude >= DESCENT_CONTOUR_ALTITUDE {
                debug_string!(
                    "Constant Acceleration Mode, Altitude: {:.2} ft",
                    altitude / FT_IN_M
                );
            ...
            let delta_thrust = if altitude >= ENGINE_CUTOFF_ALTITUDE {
                ...
                debug_string!(
                    "Terminal Descent, Altitude: {:.2} ft, Current vel: {:.2} ft/s",
                    altitude / FT_IN_M,
                    self.get_surface_approach_vel() / FT_IN_M
                );
                delta_th
            } else {
                // Turn off engines
        ...
        let (F_1, F_2, F_3, th1) =
            self.compute_thrust_from_ang_acc(vehicle_mass, reference_thrust, &angular_acc, 0.05);
    ...
    fn on_load_param(&mut self, param_data: &str) -> bool {
        if param_data.starts_with("DESCENT_PHASE") {
            ...
            if let Ok(descent_phase) = DescentPhase::from_str(descent_phase_str) {
                ...
                error!(
                    "Failed to parse DESCENT_PHASE from scenario: {}",
                    descent_phase_str
                );
            }
            true
